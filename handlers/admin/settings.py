        ...
        with db.get_connection() as conn:
            ...
            for product in products:
                cursor.execute("""
                    ...
                    VALUES (?, ?, 0)
                """, (seller_db_id, product['id']))
            
            cursor.execute("INSERT INTO seller_debt (seller_id, total_debt) VALUES (?, 0)", (seller_db_id,))
            cursor.execute("INSERT INTO seller_pending (seller_id, pending_amount) VALUES (?, 0)", (seller_db_id,))
...
# ... (пропущены функции seller_edit_code, seller_edit_name, seller_edit_tg, seller_list, seller_edit, seller_toggle_status, seller_delete, seller_confirm_delete, seller_cancel – они остаются без изменений, как в предыдущей версии)
...
async def settings_products(update: Update, context):
    ...
    with db.get_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT id, product_name, price FROM products ORDER BY product_name")
        products = cursor.fetchall()
...
async def product_add_name(update: Update, context):
    ...
    await update.message.reply_text(
        f"✅ Название принято: {product_name}\n\nШаг 2 из 2 - Введите **цену** товара (в рублях):\nНапример: 250, 300, 150",
        parse_mode='Markdown'
    )
...
async def product_confirm(update: Update, context):
    ...
    try:
        with db.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("INSERT INTO products (product_name, price, is_active) VALUES (?, ?, 1)",
            ...
            sellers = cursor.fetchall()
            for seller in sellers:
                cursor.execute("INSERT INTO seller_products (seller_id, product_id, quantity) VALUES (?, ?, 0)",
        ...
        await query.edit_message_text(
            f"✅ Товар успешно добавлен!\n\nНазвание: {product_name}\nЦена: {product_price} руб\n\nТовар добавлен всем продавцам и на центральный склад.\n\nВыберите раздел:",
            reply_markup=reply_markup
        )
        return MAIN_MENU
...
async def product_edit_start(update: Update, context):
    ...
    await query.edit_message_text(
        f"✏️ Редактирование товара\n\nТекущее название: {product['product_name']}\nТекущая цена: {product['price']} руб\n\nВыберите действие:",
        reply_markup=reply_markup
    )
    return EDIT_PRODUCT
...
async def product_change_price(update: Update, context):
    ...
    if not product_id:
        await query.edit_message_text("❌ Ошибка: товар не выбран", reply_markup=InlineKeyboardMarkup([[
            InlineKeyboardButton("🔙 К товарам", callback_data="settings_products")
        ]]))
        return PRODUCTS_MENU
...
async def product_update_field(update: Update, context):
    ...
    if editing_field == 'price':
        ...
        if len(new_value) < 2:
            await update.message.reply_text("❌ Название должно содержать хотя бы 2 символа\nПопробуйте снова:",
                                            reply_markup=InlineKeyboardMarkup([[
                                                InlineKeyboardButton("❌ Отмена", callback_data="product_cancel_edit")
                                            ]]))
            return EDIT_PRODUCT
        with db.get_connection() as conn:
            ...
            if cursor.fetchone():
                await update.message.reply_text(f"❌ Товар с названием '{new_value}' уже существует\nВведите другое название:",
                                                reply_markup=InlineKeyboardMarkup([[
                                                    InlineKeyboardButton("❌ Отмена", callback_data="product_cancel_edit")
                                                ]]))
                return EDIT_PRODUCT
...
async def product_delete(update: Update, context):
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    await query.edit_message_text(
        "⚠️ Вы уверены, что хотите удалить товар?\nЭто действие необратимо! Товар будет удален у всех продавцов и с центрального склада.",
        reply_markup=reply_markup
    )
    return EDIT_PRODUCT
...
async def product_confirm_delete(update: Update, context):
    ...
    try:
        with db.get_connection() as conn:
            ...
            cursor.execute("DELETE FROM central_stock WHERE product_id = ?", (product_id,))  # удаляем с центрального склада
...
# ... (остальные функции для товаров и бэкапов остаются как в предыдущей версии)
